...
// 新增 PTComparator，用于概率排序（大概率优先）
struct PTComparator {
    bool operator()(const PT &a, const PT &b) const {
        return a.prob < b.prob; // a 概率小于 b，则 a 排在后面
    }
};
...
public:
    ...
    // 计算一个 pt 的概率
    void CalProb(PT &pt);
    // 优先队列的初始化（将所有初始 pt push 入优先队列）
    void init();
    // 对优先队列的一个 pt 生成猜测及扩展新的 pt
    //void Generate(PT pt);
    void Generate(const PT& pt, vector<string>& local_guesses);
    // 修改后的 PopNext，使用堆顶进行展开，然后将新 pt push 入堆中
    void PopNext();
    void PopNext(int batch_size);
    // GPU接口
    void Generate_GPU(const vector<PT>& pts, vector<string>& guesses_out);
    void Generate_Serial_Optimized(const vector<PT>& pts, vector<string>& guesses_out);

    int total_guesses = 0;
    vector<string> guesses;
    vector<int> last_local_counts;  // 记录每个进程的 local_count
};
